...
public class ContactRepository implements AppRepository<Contact> {
    ...
    public void update(Long id, Contact contact) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            // Транзакция стартует
            transaction = session.beginTransaction();
            String hql = "UPDATE Contact SET name = :name, phone = :phone" +
                    " WHERE id = :id";
            MutationQuery query = session.createMutationQuery(hql);
            query.setParameter("name", contact.getName());
            query.setParameter("phone", contact.getPhone());
            ...
            LOGGER.log(Level.WARNING, "Error updating contact", e);
        }
    }
...
*/
    ...
    /*public Optional<List<Contact>> fetchLastNameIn(String lastName1, String lastName2) {
        String hql = "FROM Contact u WHERE u.lastName IN (:lastName1, :lastName2)";
        try (Session session =
                     HibernateUtil.getSessionFactory().openSession()) {
            Transaction transaction;
            // Транзакція стартує
